...
public static class AvoidanceSystem
{
    public static Vector2 PredictiveAvoidanceVector(Unit mover, List<Unit> others, Vector2 desiredDirection, out Vector2 avoidanceTarget, out bool isDetouring, out Unit? avoidanceThreat)
    {
        ...
        isDetouring = false;
        float moverRadius = mover.Radius * GameConstants.COLLISION_RADIUS_SCALE;
        ...
        foreach (var other in others)
        {
            ...
            {
                float collisionWindow = MathF.Min((combinedRadius * 2f) / minSpeed, GameConstants.AVOIDANCE_MAX_LOOKAHEAD);
            ...
            float timeWindow = MathF.Min((combinedRadius * 2f) / minSpeed, GameConstants.AVOIDANCE_MAX_LOOKAHEAD);
            ...
            float lookaheadDistance = mover.Speed * GameConstants.AVOIDANCE_MAX_LOOKAHEAD + combinedRadius;
        ...
        for (int i = 0; i <= GameConstants.MAX_AVOIDANCE_ITERATIONS; i++)
        {
            var offsets = i == 0 ? new float[] { 0f } : new float[] { GameConstants.AVOIDANCE_ANGLE_STEP * i, -GameConstants.AVOIDANCE_ANGLE_STEP * i };
    ...
    private static List<Vector2> BuildSegmentedAvoidancePath(Unit mover, Vector2 baseDir, (Vector2 relPos, float distance, float combinedRadius, Unit threat) primaryRisk)
    {
        ...
        int segmentCount = GameConstants.AVOIDANCE_SEGMENT_COUNT;
        if (segmentCount <= 0)
        {
            return path;
        }
        ...
        float startDistance = MathF.Max(GameConstants.AVOIDANCE_SEGMENT_START_DISTANCE, mover.Radius);
        float lateralDistance = primaryRisk.combinedRadius + GameConstants.AVOIDANCE_LATERAL_PADDING;
        float parallelDistance = MathF.Max(primaryRisk.distance, mover.Radius * 2f) * GameConstants.AVOIDANCE_PARALLEL_DISTANCE_MULTIPLIER;
